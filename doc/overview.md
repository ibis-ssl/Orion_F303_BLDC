...
- エンコーダー: AS5047P、SPI、14bit角度を16bitスケールへ展開。
...
速度係数校正は、固定Vqを静止状態から直接上げる方式を使わない。低速域では静止摩擦やコギングが支配的で過電流になりやすいためである。
...
| 過負荷過電流 | `BLDC_OVER_LOAD` `0x0008` | `2.00 A` | 各モーターの電源ライン電流が20ms連続で上回る | 従来の平均過電流保護 |
| 推定過負荷 | `BLDC_OVER_LOAD` `0x0008` | `3.00 A相当の短期I2R損失` | 推定電流から計算した短期損失積算がしきい値を超える | Vqと実測速度から疑似判定 |
| モーター過熱 | `BLDC_MOTOR_OVER_HEAT` `0x0004` | `70 degC` | 各モーター温度が上回る | `120 degC` 以上は未接続扱いで無視 |
| FET過熱 | `BLDC_FET_OVER_HEAT` `0x0040` | `80 degC` | 各FET温度が上回る | `120 degC` 以上は未接続扱いで無視 |
...
過負荷保護は2系統で行う。1つ目は従来の電源ライン電流が2Aを20ms連続で超えた場合の停止。2つ目は、Vq電圧と現在回転数から疑似電流を推定し、電圧指令を制限しつつ損失を積算する方式である。
