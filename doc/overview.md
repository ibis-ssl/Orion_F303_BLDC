...
FOC診断モードでは、TIM割り込み内で選択したセンサ角に `enc_offset.zero_calib + manual_offset` を加えた角度を使う。UART `B` で角度ソースを `legacy`、`raw+`、`raw-` の順に切り替える。`legacy` は現行 `as5047p.output_radian`、`raw+` はAS5047P rawを正方向に電気角化したもの、`raw-` はrawを逆方向に電気角化したものである。

UART `T` で `Uq` のトルク方向係数を `+1.0` / `-1.0` へ切り替える。初期値は `dev/simple_foc` の `APP_SENSOR_TORQUE_DIRECTION = -1.0f` に合わせて `-1.0` とする。停止時は60秒フリーウィールへ戻す。
