...
通常運転の速度指令は正逆転とも20 rpm（`1/3 rps`）を上限とする。CAN、UARTおよび基板スイッチからの指令に共通で適用する。エンコーダ値の異常飛び判定は速度指令上限とは独立させ、従来相当の160 rpsを維持する。
