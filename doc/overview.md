...
`v` 診断では、FOC用ゼロ角規約の切り分けとして `raw+z+`、`raw+z-`、`raw-z+`、`raw-z-` も出力する。これはそれぞれ `raw+ + zero`、`raw+ - zero`、`raw- + zero`、`raw- - zero` を正規化した電気角である。dev/simple_foc相当の規約は `raw+ - zero` に近いため、M0/M1でFOCの成立条件がずれる場合はこの行を比較する。
...
FOC診断モードでは、TIM割り込み内で選択したセンサ角に `enc_offset.zero_calib + manual_offset` を加えた角度を使う。UART `B` でM0/M1両方の角度ソースを、`X` でM0のみ、`Z` でM1のみを `legacy`、`raw+`、`raw-` の順に切り替える。`legacy` は現行 `as5047p.output_radian`、`raw+` はAS5047P rawを正方向に電気角化したもの、`raw-` はrawを逆方向に電気角化したものである。

UART `T` でM0/M1両方の `Uq` トルク方向係数を、`Y` でM0のみ、`H` でM1のみを `+1.0` / `-1.0` へ切り替える。初期値は `dev/simple_foc` の `APP_SENSOR_TORQUE_DIRECTION = -1.0f` に合わせて両モーターとも `-1.0` とする。停止時は60秒フリーウィールへ戻す。

実機確認では、現在Flashに保存されている `zero_calib` は legacy 規約の `raw- + zero` と一致する。そのため、この校正値をそのまま使うFOC診断モードの初期値は `raw- / -1` とする。dev/simple_foc相当の `raw+ - zero` 規約へ移行する場合は、ゼロ角校正値の保存規約も合わせて変更する必要がある。

FOC診断モードでは、dev/simple_focと同じ速度比例の位相進角モデル `-0.003457 rad/rps` を入れる。実際に電気角へ加える進角は、速度指令の符号に応じて反転する。UART `[` / `]` で位相進角トリムを `1deg` ずつ調整し、`P` でトリムを0へ戻す。低速では回るが速度を上げると崩れる場合は、まずこのトリムでM0/M1両方の傾向を見る。
