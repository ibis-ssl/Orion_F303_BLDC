# Orion_F303_BLDC 作業ログ

この文書は、実装中の試行錯誤、実機評価、測定結果を時系列で残す作業ログである。今後の開発に必要な現行仕様と操作手順は [overview.md](overview.md) に集約する。
...
## 2026-05-03 追記: ISR内軽量化と実測結果

各モーター実効更新レートを上げる前段階として、TIM1 ISR内の非必須処理を削減した。電流制限は `1.00 A` のまま変更しない。

変更点:

- AS5047P更新時に毎回行っていた診断用の `enc_elec_raw` / `output_radian` 計算をISR経路から削除した。
- AS5047Pの `diff_min` / `diff_max` 更新をISR経路から削除し、通常制御では `diff_enc` のみ更新する。
- AS5047P CS制御を `HAL_GPIO_WritePin()` から `GPIOB->BSRR` 直接操作へ変更した。
- ISR用ADC更新 `adcUpdateFast()` を `adc.h` に `static inline` で追加し、Injected ADCのJDR読み取りと `JADSTART` を直接レジスタ操作にした。
- ISR内のバッテリー電圧取得を `adcBatteryVoltageFast()` に置き換え、外部関数呼び出しを避けた。
- 速度推定で `diff / ENC_CNT_MAX / dt` としていた計算を、起動時に `SystemCoreClock / ENC_CNT_MAX` を事前計算し、ISRでは除算回数を減らす形にした。

実測結果:

| 条件 | 変更前 | 変更後 |
|---|---:|---:|
| Freewheel時 TIM1 ISR | `18/31 us` | `8/8 us` |
| センサー角Vq RUN M0/M1 `+10 rps` | 約 `27/31 us` 相当 | `16/18 us` |
| センサー角Vq RUN M0/M1 `+30 rps` | 約 `27/31 us` 相当 | `15/18 us` |
| 1kHz通常処理 M0/M1 `+30 rps` | 約 `236 us max` | `133 us max` |

現在のTIM1 update周期は約 `43.5 us` で、各ISRではM0/M1を交互に更新している。変更後のISR最大 `18 us` に対する時間余裕は約 `25.5 us`、割合で約 `59%`。TIM1 updateを2倍の約 `46 kHz` に上げた場合、周期は約 `21.8 us` となり、現状の最大 `18 us` なら時間余裕は約 `3.8 us`、割合で約 `17%` になる。2倍化は届き得るが、保護処理やデバッグ出力、割り込み競合の余裕は小さいため、次に周期を上げる場合は段階的に実測する。

なお、診断用の `enc_elec_raw` / `output_radian` / `diff_min` / `diff_max` は通常ISRでは更新しなくなったため、I/Oチェックでこれらを見る場合は最新値ではない。必要なら診断コマンド実行時だけ別途計算する。

## 2026-05-03 追記: PWM 40kHz化と高回転確認

TIM1/TIM8のPWM周期を40kHzへ変更した。TIM1 update ISRは40kHz、M0/M1はISRごとに交互更新のため、各モーターの電気レベル更新は実効20kHzとなる。

設定:

| 項目 | 値 |
|---|---:|
| TIM1/TIM8 Prescaler | `1` |
| TIM1/TIM8 Period | `899` |
| `TIM_PWM_CENTER` | `450` |
| `APP_PWM_ISR_PER_1MS` | `40` |
| 実効モーター更新 | 約 `20 kHz/motor` |

40kHz化後の処理時間:

| 条件 | ISR | 1kHz loop |
|---|---:|---:|
| Freewheel | `8/8 us` | `83/111 us` |
| センサー角Vq M0/M1 `+30 rps` | `16/18 us` | `227/350 us` |
| センサー角Vq M0/M1 `+50 rps`, Vq上限10V | `16/18 us` | `228/362 us` |
| センサー角Vq M0/M1 `+60 rps`, Vq上限12V | `17/18 us` | 約 `229/351 us` |

Vq上限と高回転確認:

| Vq上限 | 指令 | M0実測/電流 | M1実測/電流 | 結果 |
|---:|---:|---:|---:|---|
| `8 V` | `+40 rps` | `+33.57 rps`, `+0.26 A` | `+34.68 rps`, `+0.20 A` | 電圧制限で頭打ち |
| `8 V` | `+50 rps` | `+34.11 rps`, `+0.32 A` | `+35.69 rps`, `+0.21 A` | 電圧制限で頭打ち |
| `8 V` | `+60 rps` | `+34.01 rps`, `+0.28 A` | `+35.83 rps`, `+0.18 A` | 電圧制限で頭打ち |
| `10 V` | `+50 rps` | `+39.12 rps`, `+0.57 A` | `+40.63 rps`, `+0.42 A` | Faultなし |
| `12 V` | `+40 rps` | `+35.60 rps`, `+0.07 A` | `+34.64 rps`, `+0.13 A` | Faultなし |
| `12 V` | `+50 rps` | `+41.74 rps`, `+0.25 A` | `+40.69 rps`, `+0.47 A` | Faultなし |
| `12 V` | `+60 rps` | `+47.56 rps`, `+0.70 A` | `+46.18 rps`, `+0.93 A` | 1Aに近いため停止 |

40kHz化により、同じVqでの高回転時電流は以前より安定した。ただし12V/60rps指令ではM1が `0.93 A` まで上がるため、1A制限維持のままVq上限をこれ以上上げて60rpsへ進めるのは行わない。現状の安全確認済み上限は、両モーター同時ではVq上限12V時の実測約 `46から48 rps` 付近である。

## 2026-05-03 追記: 進角調整実装と40rpsまでの掃引

センサー角Vq制御用に進角調整を追加した。UART `[` で `-5 deg`、`]` で `+5 deg`、`p` で `0 deg` に戻す。進角はセンサー角Vq制御時だけ電気角へ加算し、負回転時は符号を反転する。オープンループ制御には適用しない。

注意点:

- 現状は速度閉ループではなく、Vqモデルに基づく電圧フィードフォワードである。
- そのため進角を変えると、同じ回転数指令でも実回転数が大きく変わる。
- 走行中に進角を大きくジャンプさせると過電流を誘発する。測定時は停止中に進角を設定してからランプアップする。
- 正方向進角は高回転で電流増加、失速、過電流フォルトへ寄る。負方向進角が高回転側で有効。

測定条件:

- PWM 40kHz、各モーター実効20kHz更新
- Vq上限 `12 V`
- 電流制限 `1.00 A`
- M0/M1同時駆動
- 各測定点ごとに停止、進角設定、センサー角Vqモードでランプアップ

主な測定結果:

| 指令 | 進角 | M0実測/電流 | M1実測/電流 | 結果 |
|---:|---:|---:|---:|---|
| `10 rps` | `-20 deg` | `11.82 rps`, `0.01 A` | `11.59 rps`, `0.02 A` | 電流最小相当 |
| `10 rps` | `0 deg` | `10.38 rps`, `0.03 A` | `10.29 rps`, `0.01 A` | 電流差は小さい |
| `20 rps` | `-10 deg` | `21.91 rps`, `0.03 A` | `20.96 rps`, `0.01 A` | 良好 |
| `20 rps` | `0 deg` | `19.47 rps`, `0.01 A` | `18.68 rps`, `0.03 A` | 電流最小相当 |
| `20 rps` | `+20 deg` | `14.70 rps`, `0.30 A` | `14.08 rps`, `0.43 A` | 悪化 |
| `20 rps` | `+30 deg` | - | - | `BLDC_OVER_CURRENT` |
| `30 rps` | `-20 deg` | `38.78 rps`, `0.17 A` | `40.80 rps`, `0.17 A` | 速度上昇、電流許容 |
| `30 rps` | `-10 deg` | `31.53 rps`, `0.01 A` | `32.09 rps`, `0.04 A` | 電流最小 |
| `30 rps` | `0 deg` | `26.68 rps`, `0.13 A` | `27.33 rps`, `0.03 A` | 速度不足 |
| `30 rps` | `+10 deg` | `22.69 rps`, `0.38 A` | `22.96 rps`, `0.45 A` | 悪化 |
| `30 rps` | `+20 deg` | - | - | `BLDC_OVER_CURRENT` |
| `40 rps` | `-20 deg` | `56.58 rps`, `0.15 A` | `47.11 rps`, `0.19 A` | 速度が大きく上振れ |
| `40 rps` | `-10 deg` | `42.74 rps`, `0.13 A` | `38.26 rps`, `0.09 A` | 40rps近辺で最良 |
| `40 rps` | `0 deg` | `35.11 rps`, `0.08 A` | `31.49 rps`, `0.41 A` | M1電流が増える |
| `40 rps` | `+10 deg` | - | - | `BLDC_OVER_CURRENT` |

暫定推奨:

| 指令域 | 進角 |
|---:|---:|
| `0から20 rps` | `0 deg` から `-10 deg` |
| `20から40 rps` | `-10 deg` |
| 40rps超を狙う調査時 | `-10 deg` を基準。`-20 deg` は速度上振れが大きいため注意。 |

次の段階では、単なる固定進角ではなく、実回転数または電気角速度に応じた進角テーブルを導入する。ただし、進角で実回転数も変化するため、正確に「各実回転数で電流最小」を求めるには、速度閉ループまたはVq再調整付きの掃引が必要である。

## 2026-05-03 追記: 符号規約の固定と再発防止

符号系、ゼロ角、進角は過去に何度か取り違えが発生したため、この節を最新の正とする。古い測定メモと矛盾する場合は、この節とコード中の `APP_SENSOR_DIRECTION` 周辺コメントを優先する。

正方向の定義:

- UART正方向指令: M0は `q`、M1は `e`、両軸は `w`。
- 正方向指令時の合格条件: 診断ページ `2` / `3` の `rps` が正になる。
- AS5047Pセンサー方向診断 `v` の合格条件: 電気角正方向スキャン時の `forward diff` がM0/M1とも正になり、`match 1` になる。
- 電流値は電源ラインのローパス付きセンスなので制御には使わず、過電流保護と傾向確認にだけ使う。

現在の確定値:

| 項目 | 値 | 意味 |
|---|---:|---|
| `APP_POLE_PAIRS` | `12` | 機械1回転あたり12電気回転 |
| `APP_SENSOR_DIRECTION` | `+1` | AS5047P raw増加を正方向として扱う |
| `APP_OPEN_LOOP_DIRECTION` | `-1` | open-loop電気角を正指令で負方向へ進める |
| `APP_SENSOR_TORQUE_DIRECTION` | `-1.0` | センサー角Vq制御時だけ `Uq` を反転する |
| 基準進角 | `-10 deg` | 20から40rpsで電流が小さくなりやすい実測値 |

`APP_SENSOR_DIRECTION` と `APP_SENSOR_TORQUE_DIRECTION` は別物として扱う。センサー方向が合っていても、相順とSimpleFOC式の `Uq` 符号が合わない場合がある。実際に今回のハードでは、センサー方向は `+1` で合っていたが、センサー角Vq制御では `Uq` 反転が必要だった。

相順、エンコーダ配線、ゼロ角計算、FOC式、進角式、PWMタイマーを変更した場合は、以下をこの順に実行する。

1. UART `u` で電流オフセットを再取得する。
2. UART `y` / `h` でM0/M1のゼロ角を再アラインする。
3. UART `v` でセンサー方向診断を実行し、M0/M1とも `match 1` であることを確認する。
4. UART `c`, `n` のセンサー角Vqモードで、M0 `q q`、M1 `e e` を個別に実行し、`rps` が正になることを確認する。
5. 同じく `a a` / `d d` で負方向指令時に `rps` が負になることを確認する。
6. UART `p` で進角を0degへ戻し、`[` / `]` の掃引は必ず停止中に設定してからランプアップする。
7. 10rps以下で電流が大きく増える場合は、Vqや電流制限を上げず、符号・ゼロ角・相順を疑う。

禁止事項:

- `APP_SENSOR_DIRECTION` を変えてセンサー角Vqのトルク符号を直そうとしない。
- `APP_OPEN_LOOP_DIRECTION` を変えてセンサー角Vqの符号を直そうとしない。
- ゼロ角計算に `APP_ALIGN_ELECTRICAL_ANGLE` を足し引きしない。`focSetPhaseVoltageSine(Uq, 0, angle)` 側がq軸電圧を作るため、ここで補正するとd軸寄りになりやすい。
- 走行中に大きな進角ジャンプを入れない。過電流フォルトを誘発する。

## 2026-05-03 追記: 進角1度刻み評価

UART `[` / `]` の進角調整を `5 deg` 刻みから `1 deg` 刻みに変更した。粗掃引で有効だった範囲に絞り、各測定点ごとに停止、進角設定、センサー角Vqモードでランプアップして評価した。
...
- 電流制限 `1.00 A`
- M0/M1同時駆動
- 電流はオフセット近傍で符号が揺れるため、評価では絶対値を見る

測定結果:

| 指令 | 進角 | M0実測/電流 | M1実測/電流 |
|---:|---:|---:|---:|
| `10 rps` | `-12 deg` | `11.04 rps`, `0.03 A` | `11.06 rps`, `0.01 A` |
| `10 rps` | `-10 deg` | `11.03 rps`, `0.03 A` | `10.84 rps`, `0.01 A` |
| `10 rps` | `-8 deg` | `10.81 rps`, `0.03 A` | `10.70 rps`, `0.01 A` |
| `10 rps` | `-6 deg` | `10.54 rps`, `0.03 A` | `10.48 rps`, `0.01 A` |
| `20 rps` | `-6 deg` | `20.60 rps`, `0.03 A` | `20.53 rps`, `0.01 A` |
| `20 rps` | `-4 deg` | `20.27 rps`, `0.01 A` | `20.20 rps`, `0.01 A` |
| `20 rps` | `-3 deg` | `20.06 rps`, `0.03 A` | `20.07 rps`, `0.01 A` |
| `20 rps` | `0 deg` | `19.45 rps`, `0.01 A` | `19.44 rps`, `0.02 A` |
| `30 rps` | `-13 deg` | `33.05 rps`, `0.01 A` | `33.47 rps`, `0.08 A` |
| `30 rps` | `-10 deg` | `31.64 rps`, `0.01 A` | `31.59 rps`, `0.04 A` |
| `30 rps` | `-8 deg` | `30.63 rps`, `0.05 A` | `30.69 rps`, `0.04 A` |
| `30 rps` | `-7 deg` | `29.97 rps`, `0.01 A` | `30.32 rps`, `0.04 A` |
| `40 rps` | `-13 deg` | `42.64 rps`, `0.09 A` | `42.71 rps`, `0.11 A` |
| `40 rps` | `-12 deg` | `41.69 rps`, `0.01 A` | `42.13 rps`, `0.09 A` |
| `40 rps` | `-10 deg` | `40.36 rps`, `0.03 A` | `40.67 rps`, `0.05 A` |
| `40 rps` | `-9 deg` | `39.86 rps`, `0.02 A` | `39.71 rps`, `0.09 A` |
| `40 rps` | `-7 deg` | `38.09 rps`, `0.01 A` | `38.53 rps`, `0.07 A` |

実回転数の目標近さと電流の小ささを両方見ると、暫定推奨は次の通り。

| 指令域 | 推奨進角 | 理由 |
|---:|---:|---|
| `10 rps` | `-6 deg` | 電流差は小さいが、実回転数の上振れが比較的小さい |
| `20 rps` | `-3 deg` から `-4 deg` | 実回転数が目標に近く、電流も低い |
| `30 rps` | `-7 deg` | 実回転数が目標に近く、電流も低い |
| `40 rps` | `-9 deg` から `-10 deg` | 実回転数が目標に近く、電流も低い |

この結果から、固定 `-10 deg` は30から40rpsでは有効だが、20rps以下ではやや強い。速度に応じた進角テーブルを入れるなら、`10 rps: -6 deg`, `20 rps: -4 deg`, `30 rps: -7 deg`, `40 rps: -10 deg` を初期値にする。

## 2026-05-03 追記: 進角モデルのフィッティング

1度刻み掃引の最小電流点は、電流センスのオフセット揺れ、機械摩擦、実回転数のずれ、停止からのランプアップ状態に引っ張られる。そのため、最小値をそのままテーブル化せず、進角の効果原理に基づいてフィットする。

進角は主に、電気角速度に対する制御遅れ、PWM反映遅れ、センサー取得遅れ、電流応答遅れを補償するものとして扱う。したがってモデルは次の制約を持つ。

- `0 rps` では進角 `0 deg`
- 高速ほど進角絶対値が大きくなる
- このハードでは有効方向は負方向
- ランダムな単一点ではなく、20から40rpsの傾向を優先する

20/30/40rps付近の有効点を使い、原点拘束付き最小二乗で次のモデルにした。

```text
phase_advance_deg = -0.2379 * abs(target_rps)
phase_advance_rad = -0.00415268 * abs(target_rps)
```

これは12極対の電気角速度に対して、約 `55 us` の等価遅れを補償するモデルである。

```text
tau = 0.2379 deg/rps / (360 deg * 12 pole-pairs) = 55 us
```
...
実装では `APP_PHASE_ADVANCE_MODEL_RAD_PER_RPS = -0.00415268f` とし、UART `[` / `]` はモデル値そのものではなく1deg単位のトリムとして扱う。UART `p` はトリムを0degへ戻す。

モデル実装後の確認結果:

| 指令 | モデル進角 | M0実測/電流 | M1実測/電流 |
|---:|---:|---:|---:|
| `10 rps` | `-2.4 deg` | `10.89 rps`, `0.03 A` | `10.20 rps`, `0.01 A` |
| `20 rps` | `-4.8 deg` | `21.92 rps`, `0.01 A` | `20.21 rps`, `0.01 A` |
| `30 rps` | `-7.1 deg` | `33.55 rps`, `0.01 A` | `29.89 rps`, `0.04 A` |
| `40 rps` | `-9.5 deg` | `45.98 rps`, `0.08 A` | `39.53 rps`, `0.06 A` |

M0/M1で実回転数差は残るため、これは最終的な速度精度モデルではない。現段階の目的は、ランダムな最小電流点ではなく、制御遅れに基づく進角の初期モデルを入れること。速度精度を詰める場合は、次にVqモデルまたは速度閉ループ側で補正する。

## 2026-05-04 追記: 逆回転側の進角評価

正回転側と同じ共通進角モデルを使い、逆回転でも40rpsまで評価した。実装上は、`target_rps < 0` のときセンサー角へ加算する進角符号を反転する。
...
- 電流制限 `1.00 A`
- M0/M1同時駆動
- 共通モデル `phase_advance_deg = -0.2379 * abs(target_rps)` を基準
- UART `[` / `]` によるトリムを `-3` から `+3 deg` まで掃引
- 各測定点ごとに停止、トリム設定、センサー角Vqモードで負方向へランプアップ

主な結果:

| 指令 | トリム | 表示進角 | M0実測/電流 | M1実測/電流 |
|---:|---:|---:|---:|---:|
| `-10 rps` | `0 deg` | `-2.4 deg` | `-10.34 rps`, `0.03 A` | `-10.50 rps`, `0.01 A` |
| `-10 rps` | `+2 deg` | `-0.4 deg` | `-10.28 rps`, `0.03 A` | `-10.37 rps`, `0.01 A` |
| `-20 rps` | `0 deg` | `-4.8 deg` | `-20.30 rps`, `0.03 A` | `-21.38 rps`, `0.01 A` |
| `-20 rps` | `+3 deg` | `-1.8 deg` | `-19.89 rps`, `0.01 A` | `-20.72 rps`, `0.01 A` |
| `-30 rps` | `0 deg` | `-7.1 deg` | `-29.94 rps`, `0.05 A` | `-32.33 rps`, `0.07 A` |
| `-30 rps` | `+3 deg` | `-4.1 deg` | `-28.96 rps`, `0.03 A` | `-30.86 rps`, `0.03 A` |
| `-40 rps` | `0 deg` | `-9.5 deg` | `-40.20 rps`, `0.05 A` | `-43.91 rps`, `0.12 A` |
| `-40 rps` | `+2 deg` | `-7.5 deg` | `-38.33 rps`, `0.05 A` | `-42.12 rps`, `0.06 A` |

逆回転では、正回転よりも進角絶対値を小さくした方がM1の速度上振れと電流が抑えられる傾向がある。フォルトは発生せず、40rpsまでの電流は1Aに対して十分低い。

正逆両方向を共通パラメーターで扱う場合、正回転側だけから得た `-0.2379 deg/rps` はやや強い。正回転の有効点 `20:-4`, `30:-7`, `40:-10 deg` と、逆回転の有効点 `20:-1.8`, `30:-4.1`, `40:-7.5 deg` を合わせ、原点拘束で再フィットすると次の値になる。

```text
phase_advance_deg = -0.1981 * abs(target_rps)
phase_advance_rad = -0.003457 * abs(target_rps)
```

これは12極対に対して約 `46 us` の等価遅れ補償に相当する。

```text
tau = 0.1981 / (360 * 12) = 46 us
```

共通モデルとしては、この `-0.198 deg/rps` を次候補にするのが妥当。正回転だけの最小電流へ寄せるのではなく、正逆両方向で過進角になりにくい値を採用する。
## 2026-05-04 追記: 多点エンコーダー角度校正と速度係数校正

最終校正値は、各モーターの `zero_electric_angle` と `rps_per_v` のみを保存対象とする。電源ライン電流センスはローパス付きの保護用途であり、電流制御や抵抗推定には使わない。旧実装の `voltage_offset` は起動時のレガシー値読み込みには残すが、新しい校正では常に `0.0 V` として保存する。

エンコーダー角度校正は、6点の電気角を正順と逆順の2パスで固定し、各点250msのサンプルを円周平均する。固定角はSimpleFOCのUq軸ではなくUd軸へ印加する。Uq固定角を使うと磁束軸が90度ずれ、センサ角Vq制御で静止電流が増えて回らない状態になったためである。`startSensorAlignment()` も同じUd固定角に合わせ、ゼロ角は `sensor_direction * pole_pairs * shaft_angle - align_electrical_angle` で求める。

速度係数校正は、固定Vqを静止状態から直接上げる方式を使わない。実機確認ではM0で `+3 V` 付近に入った時点で過電流保護に入り、低速域の静止摩擦や軸ずれの影響が支配的だった。現在は通常のセンサ角電圧制御経路で `+10/+20/+30/+40/-10/-20/-30/-40 rps` を順に指令し、実際に出した `abs(Vq)` と実測 `abs(rps)` から原点通過の最小二乗で `voltage_per_rps` を求める。フィット後は5%の余裕を掛ける。

進角は正逆共通モデルとして `APP_PHASE_ADVANCE_MODEL_RAD_PER_RPS = -0.003457f` を使う。これは正転だけの最小電流点ではなく、正逆40rpsまでの傾向を合わせた値であり、校正中の速度掃引にも適用する。

2026-05-04の保存なし校正 `k` の実機確認結果:

| 項目 | M0 | M1 |
|---|---:|---:|
| エンコーダーゼロ角 | `+5.808 rad` | `+3.027 rad` |
| 速度係数 | `+0.202 V/rps` | `+0.203 V/rps` |
| 換算値 | `+4.940 rps/V` | `+4.929 rps/V` |
| フィット点数 | `8` | `8` |

校正後のRAM値で `c`, `n`, `w` x20 によりセンサ角電圧モードの10rps動作を確認した。結果は M0 `+10.16 rps / 0.03 A`, M1 `+10.15 rps / 0.08 A` で、Faultは発生していない。

同じ手順をFlash保存ありの `K` でも実行し、保存後にST-Linkリセットして読み戻しを確認した。ブートログは `zero +5.808 +3.118`, `off +0.00 +0.00`, `v/rps +0.203 +0.204`。保存値で再起動後、`c`, `n`, `w` x20 の10rps動作は M0 `+10.31 rps / 0.05 A`, M1 `+10.35 rps / 0.08 A` で、Faultは発生していない。

## 2026-05-04 追記: ゼロ電気角校正の機械1回転化

40rps正逆試験でM1の正逆差が大きかったため、ゼロ電気角校正を改善した。従来は6点の電気角を正逆2パスで測っていたが、AS5047P磁石偏心、機械角依存のコギング、取り付け誤差を平均化しきれない可能性があった。改善後は `6点/電気周期 * 12極対 = 72点` を正順と逆順で測り、機械1回転分のゼロ角を円周平均する。

各点ではUd固定角へ250ms整定し、100msサンプルする。正順と逆順の同一点ペアからヒステリシスを計算し、`8deg` を超える点はゼロ角平均から除外する。校正ログには `pairs used/rej` と `hyst avg/max` を出す。2026-05-04時点の実機では全点採用でき、最大ヒステリシスも1deg未満だった。

保存なし `k` の評価結果:

| 項目 | M0 | M1 |
|---|---:|---:|
| ゼロ角 | `+5.797 rad` | `+3.066 rad` |
| ペア採用/除外 | `72/0` | `72/0` |
| ヒステリシス平均/最大 | `0.1/0.3 deg` | `0.2/0.5 deg` |
| 速度係数 | `+0.203 V/rps` | `+0.204 V/rps` |

このRAM値で40rps正逆を再評価した結果:

| 条件 | M0平均 | M1平均 |
|---|---:|---:|
| `+40 rps` | `+41.83 rps`, `0.10 A` | `+41.92 rps`, `0.14 A` |
| `-40 rps` | `-41.82 rps`, `0.10 A` | `-41.72 rps`, `0.14 A` |

改善前はM1が `+44.03 rps` と `-39.26 rps` 程度までずれていたが、機械1回転校正後は正逆差が大きく縮小した。この結果から、主因はモーター定数のCW/CCW差ではなく、ゼロ電気角の機械角依存誤差を十分に平均化できていなかったことと判断する。

保存あり `K` でも同じ校正を実行し、ST-Linkリセット後のブートログで `zero +5.795 +3.067`, `off +0.00 +0.00`, `v/rps +0.203 +0.203` の読み戻しを確認した。保存値での簡易40rps確認では M0/M1 が `+42.04/+42.06 rps`, `-41.74/-41.91 rps` で、Faultは発生していない。
