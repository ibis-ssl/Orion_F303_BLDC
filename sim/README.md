...
ライブ実行は、実機20kHz ISR相当の1ステップ `50us` をGUI上で `50ms` ごとに進めるため、シミュレーション時間は実時間の `0.1%` で進む。実行中も速度指令、角度規約、ゼロ角、エンコーダ遅延、位相トリムなどの入力値を変更できる。エンコーダは機械角を参照し、M0/M1のPWM出力は前回値を保持したまま物理モデルを進める。
