...
& python (Join-Path $root "Script\stamp_fw_version.py") $elf --objcopy (Join-Path $ToolchainBin "arm-none-eabi-objcopy.exe") --repo $root --target bldc --log-dir (Join-Path $root "Script\Logs\Build");if($LASTEXITCODE){throw "FW version stamping failed"}
